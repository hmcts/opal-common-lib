...
This file covers repo-specific guidance for `opal-common-lib`.
...
## Local Notes
- This repo is a library, not a runnable Spring Boot app.
- Baseline validation is usually `./gradlew clean build`.
- Use `./gradlew publishToMavenLocal` when a consuming service needs to test unpublished local changes.
