...
class ContentDigestResponseFilterTest {
    ...
    @Mock
    ContentDigestProperties.Request contentDigestRequest;
    ...
    @BeforeEach
    void setUp() {
        when(contentDigestProperties.getRequest()).thenReturn(contentDigestRequest);
    }
    ...
    class DoFilterInternal {
        ...
        private void setupAutoGenerated(boolean isAutoGenerated) {
            when(contentDigestRequest.isAutoGenerated()).thenReturn(isAutoGenerated);
        }
        ...
        void shouldAddContentDigestHeaderWhenResponseHasBody() {
            ...
            doReturn(contentDigestResponse).when(contentDigestProperties).getResponse();
            doReturn("SHA-512").when(contentDigestResponse).getAlgorithm();
            ...
            try {
                ...
                verify(contentCachingResponseWrapper).setHeader("Content-Digest",
                    "sha-512=:EjDB/yJaXXxiltpzgv1JC/kraxsja7N/H/7BwuPU0S+MKMZsgH6Lhqa4LmR3vFm1auMMGArd3eqAkEUHMnvJ6Q==:"
                );
                verifyNoMoreInteractions(chain, contentCachingResponseWrapper);
                verifyNoInteractions(request, response);
            } catch (Exception e) {
